...
if (isset($_POST['update_profile'])) {
    ...
    if (!$newProfile) {
        ...
        db_execute("UPDATE admin SET profile = ? WHERE username = ?", "ss", $newProfile, $ad);
...
if (isset($_POST['change_username'])) {
    ...
    if ($uname === '') {
        ...
        $exists = db_select_one("SELECT id FROM admin WHERE username = ? AND username != ? LIMIT 1", "ss", $uname, $ad);
        if ($exists) {
            $error = "Username already exists.";
        } else {
            db_execute("UPDATE admin SET username = ? WHERE username = ?", "ss", $uname, $ad);
...
if (isset($_POST['update_pass'])) {
    ...
    $row = db_select_one("SELECT password FROM admin WHERE username = ? LIMIT 1", "s", $ad);
    ...
    if ($oldPass === '') {
        ...
        db_execute("UPDATE admin SET password = ? WHERE username = ?", "ss", $newHash, $ad);
        $success = "Password updated successfully.";
    }
}

$row = db_select_one("SELECT username, profile FROM admin WHERE username = ? LIMIT 1", "s", $ad);
$username = $row['username'] ?? $ad;
$profile = $row['profile'] ?? 'admin.jpg';
?>
<!DOCTYPE html>
<html>
<head>
