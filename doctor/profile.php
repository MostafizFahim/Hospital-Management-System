...
if (isset($_POST['change_username'])) {
    ...
    if ($uname === '') {
        ...
        if ($exists) {
            ...
            $updatedDoctor = db_execute("UPDATE doctors SET username = ? WHERE username = ?", "ss", $uname, $doc);
            $updatedAppointments = db_execute("UPDATE appointment SET doctor_username = ? WHERE doctor_username = ?", "ss", $uname, $doc);
            $updatedIncome = db_execute("UPDATE income SET doctor = ? WHERE doctor = ?", "ss", $uname, $doc);
            $updatedPrescriptions = db_execute("UPDATE prescriptions SET doctor_username = ? WHERE doctor_username = ?", "ss", $uname, $doc);

            if ($updatedDoctor && $updatedAppointments && $updatedIncome && $updatedPrescriptions) {
...
if (isset($_POST['update_pass'])) {
    ...
    if ($oldPass === '') {
        ...
        db_execute("UPDATE doctors SET password = ? WHERE username = ?", "ss", $newHash, $doc);
...
$profile = $row['profile'] ?? 'doctor.jpg';
?>
<!DOCTYPE html>
<html>
<head>
